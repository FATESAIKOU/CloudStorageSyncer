...
package "CLI Layer" {
  ...
  class UploadCommands {
    +file(file_path, s3_key, storage_class, config_path)
    +directory(dir_path, prefix, storage_class, recursive, config_path)
    +batch(file_list, storage_class, config_path)
  }
  ...
  class DownloadCommands {
    +file(s3_key, output_path, output_dir, overwrite, config_path)
    -validate_arguments()
    -handle_output_path_conflict()
  }

  class DeleteCommands {
    +file(s3_key, force, config_path)
    -confirm_deletion(s3_key): bool
  }
}
